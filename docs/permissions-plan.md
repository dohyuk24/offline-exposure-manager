...
> **상태**: 🟡 **부분 구현** · N1 (DB 스키마) + N2 (Slack OAuth 로그인 코드) 머지. N3 (가드/RLS 강제), N4 (어드민 권한 페이지) 후순위 보류.
> **운영 source of truth (구현분)**: PRD §6 (권한 시스템) + 코드. 슬랙 OAuth 셋업 미완 (사용자 액션 대기).
> **본 문서**: 인증 방식·권한 모델 결정 history. N3/N4 진행 시 §5~§6 참조.
> **작성일**: 2026-04-26
