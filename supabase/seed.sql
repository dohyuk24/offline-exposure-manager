-- ============================================================
-- 오프라인 매체 관리 서비스 — Seed 데이터
--
-- 대상 월: 2026-04
-- 지점 14개 · 매체 레코드 50+건 · 예산/점수 로그 포함
--
-- 재실행 안전성: 하단 테이블을 TRUNCATE 후 재삽입한다.
-- ============================================================

truncate table score_logs restart identity cascade;
truncate table budget_logs restart identity cascade;
truncate table media_records restart identity cascade;
...
--    구분: 공식 / 자체보유 / 비공식
--    상태: 게시중 / 게시종료 / 협의중 / 협의실패 / 아이디어 / 미진행 / 협의완료
-- ============================================================
...
-- ============================================================
-- 3) 예산 사용 내역 (year_month = '2026-04')
--    매체 레코드 중 비공식 · 자체보유 cost를 반영 + 별도 비용
-- ============================================================
...
-- ============================================================
-- 4) 점수 내역 (year_month = '2026-04')
--    action: update / new_discovery / barter_success
--    점수: update=1, new_discovery=5, barter_success=7
-- ============================================================

-- update: 각 지점 업데이트 활동 (지점별 2~4회)
...
-- new_discovery: 신규 발굴 레코드(is_new_discovery = true) 자동 부여
...
-- barter_success: 바터제휴배너 + 게시중 상태 + barter_condition 있는 레코드
insert into score_logs (branch_id, media_record_id, action, score, year_month)
